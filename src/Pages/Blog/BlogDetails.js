...
const BlogDetails = () => {
  const {id} = useParams()
    ...
    useEffect(()=>{
     axios.get('https://raw.githubusercontent.com/anishazahan/Best-Choice/main/public/Blog.json')
      .then(data=>
       {
        const findBlog = data.data.find(blog=>blog.id === id)
  ...
  return (
    <div className='mt-20 mb-28 max-w-7xl px-5 lg:px-10'>
       <img className='mx-auto' src={blogDetails.img} alt="" />
       <h2 className='text-center font-semibold text-black my-5'>{blogDetails.tittle}</h2>
